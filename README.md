...
`mod` is a simple command line tool to initialize Deno projects from
prompts.

## Quickstart

```
deno install --allow-read --allow-run=git --allow-write --name mod https://deno.land/x/init@v0.1.0/mod.ts
...
You can install `mod` from a GitHub raw URL with a tag as well, or without
...
Note that `mod` will not overwrite files unless the `--force` option is
...
the `name` argument can be any path in the local filesystem, and `mod`
